...
class SecurityChoiceForm(forms.Form):
    ...
    AUTH_CHOICES = (
        (
            AUTH_OFF,
            "Open without a password (recommended). Anyone who can use this Windows account can open the app.",
        ),
        (
            AUTH_ON,
            "Ask for a password. This app will ask for a local app password before opening records.",
        ),
    )

    auth_enabled = forms.ChoiceField(
        label="Choose how this computer opens the app",
    ...
    password1 = forms.CharField(
        ...
        help_text="Only needed if this computer will ask for a password.",
    )
...
class FirstRunOwnerForm(SecurityChoiceForm):
    username = forms.CharField(
        ...
        max_length=150,
        initial="Owner",
        help_text="This is the local app account used when password protection is on.",
    )
...
def _save_setup_settings(auth_enabled):
...
def first_run_setup(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user.save()
            _save_setup_settings(auth_enabled)
